 ...
 *  Liberally cribbed from Joe Beeson <jbeeson@gmail.com>
 *
...
definition(
    ...
    description: "Set Arlo modes to follow SHM",
...
preferences {
    ...
    section("Arlo modes for SHM mode:") {
      input(name: "arloOff", type: "text", title: "Off")
      input(name: "arloStay", type: "text", title: "Stay")
      input(name: "arloAway", type: "text", title: "Away")
    }
}
...
def initialize() {
    log.info "Initializing; subscribe to alarmSystemStatus for $location"
...
def shmaction(evt) {
    ...
    if(evt.value == "off") {
       setArloBaseStationMode(settings.arloOff)
    }
    else if(evt.value == "stay") {
       setArloBaseStationMode(settings.arloStay)
    }
    else if(evt.value == "away") {
       setArloBaseStationMode(settings.arloAway)
    }
    else {
      log.debug "Got unrecognized ${evt.value}"
    }
}
